...
$depuni_id=$_POST["depuni_id"];
$not= iconv('UTF-8','ISO-8859-9',$_POST['depuni_notlar']);
...
SET
	...
	depuni_adi = '".$_POST['depuni_adi']."',
	depuni_tip = '".$_POST['depuni_tip']."',
	dep_uni_serino = '".$_POST['dep_uni_serino']."',
	dep_uni_disk_boyutlari = '".$_POST['dep_uni_disk_boyutlari']."',
	dep_uni_isletim_sistemi =  '".$_POST['dep_uni_isletim_sistemi']."',
	dep_uni_disk_array_bilgileri = '".$_POST['dep_uni_disk_array_bilgileri']."',
	depuni_notlar = '".$not."',
	depuni_demirbasno = '".$_POST['depuni_demirbasno']."',
	depuni_marka_model = '".$_POST['depuni_marka_model']."',
	depuni_ip = '".$_POST['depuni_ip']."',
	aktif =".$_POST['aktif']."
	WHERE depuni_id=$depuni_id;";
...
$sql_donanim_ag_bilesenleri="UPDATE
	...
	SET
	aktif =".$_POST['aktif']."
	WHERE donanim_tip=3 and donanim_tip_id=$depuni_id ;";	
...
if(isset($_POST["eth_sw"])){
$select_donanimag_eth="SELECT controller_sayisi, controller_basina_dusen_port_sayisi,port_sayisi From donanim_ag_bilesenleri WHERE port_turu =2 and donanim_tip = 3 and donanim_tip_id =".$depuni_id." ;";
...
if($row_eth["port_sayisi"]=="")
{
	...
	VALUES 
	(2,3,".$depuni_id." ,".$controller_sayisi_san." ,".$controller_basina_dusen_port_sayisi_san.",".$port_sayisi_san.");";
}
else{
	...
	WHERE port_turu =2 and donanim_tip = 3 and donanim_tip_id =".$depuni_id." ;
	";
}
	...
	if (!mysql_query($sql_update_eth_donanim_ag_bilesenleri,$link))
	{
		...
		for($j=1;$j<=$eth_controller_basina_dusen_port_sayisi;$j++){
			$sql_insert_eth_detay="INSERT INTO donanim_ag_bilesenleri_detay
				(port_turu, donanim_tip, donanim_tip_id, controller_adi, controller_port_adi, port_adi,aktif)
				VALUES 
				(2,3,".$depuni_id." ,'".$_POST["depuni_adi"]."_cnt".$i."', 'P".$j."', '',1);";
		...
		if(!mysql_query($sql_insert_eth_detay,$link)){
			...
			if($eth_port_sayisi>0)
			{
					for($i=$ii2;$i<=$eth_port_sayisi;$i++)
					{
						...
						(port_turu, donanim_tip, donanim_tip_id, controller_adi, controller_port_adi, port_adi,aktif)
						VALUES 
						(2,3,".$depuni_id." ,'', '',  '".$_POST["depuni_adi"]."-P".$i."',1);";
						$sql_insert_eth_detay=iconv('UTF-8', 'ISO-8859-9//IGNORE', $sql_insert_eth_detay);
					if(!mysql_query($sql_insert_eth_detay,$link)){
						echo "Ağ bileşeni ekleme başarısız".mysql_error();
					}
...
if(isset($_POST["san"])){
$select_donanimag_san="SELECT controller_sayisi, controller_basina_dusen_port_sayisi,port_sayisi From donanim_ag_bilesenleri WHERE port_turu =1 and donanim_tip = 3 and donanim_tip_id =".$depuni_id." ;";
...
if($row_san["port_sayisi"]=="")
{
	...
	(port_turu, donanim_tip, donanim_tip_id, controller_sayisi, controller_basina_dusen_port_sayisi, port_sayisi)
	VALUES 
	(1,3,".$depuni_id." ,".$controller_sayisi_san." ,".$controller_basina_dusen_port_sayisi_san.",".$port_sayisi_san.");";
}
else{
	...
	controller_basina_dusen_port_sayisi =".$controller_basina_dusen_port_sayisi_san." ,
	port_sayisi = ".$port_sayisi_san."
	WHERE port_turu =1 and donanim_tip = 3 and donanim_tip_id =".$depuni_id." ;
	";
}
	...
	if (!mysql_query($sql_update_san_donanim_ag_bilesenleri,$link))
	{
		...
		for($j=1;$j<=$controller_basina_dusen_port_sayisi_san;$j++){
			$sql_insert_eth_detay="INSERT INTO donanim_ag_bilesenleri_detay
				(port_turu, donanim_tip, donanim_tip_id, controller_adi, controller_port_adi, port_adi,aktif)
				VALUES 
				(1,3,".$depuni_id." ,'".$_POST["depuni_adi"]."_cnt".$i."', 'P".$j."', '',1);";
			$sql_insert_eth_detay=iconv('UTF-8', 'ISO-8859-9//IGNORE', $sql_insert_eth_detay);
		if(!mysql_query($sql_insert_eth_detay,$link)){
						echo "Ağ bileşeni ekleme başarısız--".mysql_error();
					}
			...
			if($port_sayisi_san>0)
			{
					for($i=$ii2;$i<=$port_sayisi_san;$i++)
					{
						...
						(port_turu, donanim_tip, donanim_tip_id, controller_adi, controller_port_adi, port_adi,aktif)
						VALUES 
						(1,3,".$depuni_id." ,'', '',  '".$_POST["depuni_adi"]."-P".$i."',1);";
						$sql_insert_eth_detay=iconv('UTF-8', 'ISO-8859-9//IGNORE', $sql_insert_eth_detay);
					if(!mysql_query($sql_insert_eth_detay,$link)){
						echo "Ağ bileşeni ekleme başarısız--".mysql_error();
					}
