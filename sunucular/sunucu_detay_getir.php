...
else
{
	...
	dosya_sistemi ,ip_adresi1 ,ip_adresi2 ,isletim_sistemi ,lokasyon
 	...
 	alim_tarihi ,garanti_suresi,sunucu_demirbasno FROM sunucular where sunucuID='".$sid."' " ;
	$sid2=$sid;
	//echo $sql_sunucular ;
	$result=mysql_query($sql_sunucular,$link);
	$row=mysql_fetch_array($result,MYSQL_BOTH);
...
if( isset($_SESSION['login']) ) 
{
	if($_SESSION['login_yetki']=="1"){
		...
		echo '<a title="Güncelle"  class="upload_button_blue" id="index2.php?s=s_g&sid='.$sid2.'" href="#">Güncelle</a> ';
		echo '<a title="Arşive Gönder"  class="upload_button_blue" id="index2.php?s=s_a&sid='.$sid2.'" href="#">Arşive Gönder</a>';
	}
}
