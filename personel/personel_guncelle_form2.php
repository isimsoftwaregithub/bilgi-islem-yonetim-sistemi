...
<script>
	 $(document).ready(function(){
			...
			$('#personelguncelle').on('submit', function(e)
				    {
			    	
				        e.preventDefault();
				        $('#upload_button_blue').attr('disabled', ''); // disable upload button
				        //show uploading message
				        $("#output").html('<div style="padding:10px"><img src="images/ajax-loader.gif" alt="Lütfen Bekleyin"/> <span>Güncelleniyor...</span></div>');
				        $(this).ajaxSubmit({
				        target: '#output',
	 ...
	 function afterSuccess()
	   {
		   //$('#personelguncelle').resetForm();  // reset form
...
<p class="baslik_blue">Bilgilerim</p>
<div id="theForm">
	<fieldset class="fieldset_blue">
		<legend class="legend_blue">Bilgiler</legend>
		
		<form action="personel/personel_guncelle2.php" id="personelguncelle" enctype="multipart/form-data" method="post" >
		<input type="hidden" name="uyeid" value="<?php echo $row['uye_id']?>"/>
			<table class="personel_guncelle">
				...
				<tr><th vAlign="top" scope="row" align="left"></th><td><button type="submit" id="upload_button_blue">Güncelle</button> </td></tr>
			</table>
	...
	<?php 
 		...
 		odbc_result_all($result_1,'class="personel_ekle"');
 		odbc_close_all(); 
	?>
</div>
</div>
