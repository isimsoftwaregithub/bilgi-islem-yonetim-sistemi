	...
	$where=" AND (anahtar_kelimeler like '%".$anahtar_kelime."%' OR icerik like '%".$anahtar_kelime."%' OR baslik like '%".$anahtar_kelime."%')".$where;
		...
		$url = '?olay_tip='.$olay_tip.'&olay_tip_id='.$olay_tip_id.'&anahtar_kelime=&';
           ...
           //echo $sql_olay;
			...
			echo '<tr class="'.$class.' "> <td class="'.$class.' baslik">'.$row['baslik'].'</td><td class="'.$class.' eklenmetarihi">'.$row['eklenme_tarihi'].' <p  style="cursor:hand;cursor:pointer;" class="olay_ozet" onclick="open_popup(\'olay_guncelle_form.php?oid='.$row['olay_id'].'\')" >
			';
            ...
            <tr class="'.$class.'"><td title="Anahtar Kelimeler ve Ekler" class="'.$class.' anahtarkelime">'.$row['anahtar_kelimeler'].$ekcount.'</td><td class="'.$class.' kisi">'.$row['ad'].' '.$row['soyad'].'</td></tr>';      
           	if($class=="odd")
			{
				$class="even";
			}
