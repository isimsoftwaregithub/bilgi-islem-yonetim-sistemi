...
if(isset($_GET["t"])){
$id=$_GET["t"];
$where=" AND s.sunucu_tur_id =".$id;
}

ob_start();
include 'db/mysql_baglan.php';
	$sql_sunucular="SELECT s.sunucuID,sunucu,sunucu_gorevi,
	ltrim(notlar)  notlar,
	serino,
	sunucu_demirbasno,
	cpu_turu,
...
?>	
<div id="sunucu_rapor">
<p class="baslik_sunucu_rapor">Sunucu Rapor 
...
while ($row=mysql_fetch_array($result,MYSQL_BOTH))
{
	...
	$sunucu_gorevi=$row["sunucu_gorevi"];
	if($row["sunucu"]==$row["sunucu_gorevi"]){
		$sunucu_gorevi="";
	}
//	<td  style=' background-color:$bg;width:120px;vertical-align: top'>".$row["sunucu_turu"]."</td>
	echo "<tbody><tr>
	<td  style='background-color:$bg; width:2px;vertical-align: top'>".$sira."</td>
	<td  style=' background-color:$bg;width:200px;vertical-align: top'>".$row["sunucu"]."<br/> ".$sunucu_gorevi."<br/>".$row["marka"]." ".$row["model"]."</td>
		
	
	<td  style='background-color:$bg; width:75px;vertical-align: top'>".$row["serino"]." / ".$row["sunucu_demirbasno"]."</td>
	...
	<td  style='background-color:$bg; width:85px;vertical-align: top'>".$row["ip_adresi1"]." ".$row["ip_adresi2"]."</td>
	<td  style='background-color:$bg; width:75px;vertical-align: top'>".$row["isletim_sistemi"]."</td>
	<td  style='background-color:$bg; width:200px;vertical-align: top'>".$row["vertab_adi"]."<br/>".$row["veritabani_ver"]."</td>
	
	<td  style='background-color:$bg; width:65px;vertical-align: top'>
	";
